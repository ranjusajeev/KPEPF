...
namespace KPEPFClassLibrary
{
    public class WithdrawalCDao : KPEPFDAOBase  
    {
        ...
        public DataSet FillConsAmtToKPEPF14(ArrayList ArrIn)
        {
            DataSet ds = new DataSet();
            ds = Fetch("AGBalanceTransferCr_S3", CommandType.StoredProcedure, ArrIn);//Chalan_S1
            return ds;
        }
        public DataSet FillConsAmtToGPF14(ArrayList ArrIn)
        {
            DataSet ds = new DataSet();
            ds = Fetch("AGBalanceTransferDt_S3", CommandType.StoredProcedure, ArrIn);//Chalan_S1
            return ds;
        }
        ...
        public DataSet FillClosure()
        {
            DataSet ds = new DataSet();
            ds = Fetch("Pfo_ClosureDetails_TRN_S6", CommandType.StoredProcedure);//Chalan_S1
            return ds;
        }
        public DataSet  saveBillAccDtMapping(ArrayList arb)
        {
            DataSet ds = new DataSet();
            try
            {
                ds = Fetch("Tr_Bill_Pde_I", CommandType.StoredProcedure, arb);
                return ds;
            }
            catch (Exception E)
            {
