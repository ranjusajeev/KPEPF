...
namespace KPEPFClassLibrary
{
    public class OBDao : KPEPFDAOBase
    {
        ...
        public DataSet GetObSingleCalc(ArrayList ar)
        {
            DataSet dsO1 = new DataSet();;
            dsO1 = Fetch("Pfo_ClosingBalance_S3", CommandType.StoredProcedure, ar);
            return dsO1;
        }
        ...
        public void DelOb(OB ob)
        {
            ArrayList arr = new ArrayList();
            arr.Add(ob.IntAccNo);
            try
            {
                Save("Pfo_ClosingBalance_D", CommandType.StoredProcedure, arr);
            }
            catch (Exception E)
            {
                throw new Exception("Check the Error" + E.Message);
